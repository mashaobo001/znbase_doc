...
![](./assets/reference/150a2f35743fc3b748f75330823451e0.png)
...
使用 Avro 创建一个连接到 Kafka 的 changefeed
...
![](./assets/reference/924f4a6176a60e88b4ef24cef97ac543.png)
...
![](./assets/reference/3291d37d74975a0af32a1deb7d9fe0d6.png)
...
角色功能便于对用户权限进行管理。在实际应用中针对用户进行授权费时费力。因此把有相同权限的用户组织在一起作为一个用户组，称为角色。用户只需要修改角色的权限，就可以统一管理属于这个角色的所有用户的权限。

#### 创建角色

​		CREATE ROLE语句用于创建角色。可以为角色分配权限，分配后角色的所有成员（包括直接成员和间接成员）都将继承角色的权限。下面介绍CREATE ROLE的具体使用方法。

需要注意的是:

-   角色只能由超级用户（即admin角色组成员）创建。默认情况下，root属于admin用户。

-   创建角色后，需要为其授予对数据库和表的权限。

-   角色的成员可以是用户或者其他角色。

-   角色和用户共享相同的名称空间，它们必须是唯一的。
...
-   不允许成员循环，包括直接循环和间接循环（直接循环：A是B角色的成员，B是A角色的成员；间接循环：A是B角色的成员，B是C角色的成员，C是A角色的成员）。
...
![](./assets/reference/20d112286e1641c803743dbc72657eca.png)

若CREATE ROLE不使用IF NOT EXISTS，如果不存在要创建的角色名称，则创建角色；否则报错：角色已存在。

若CREATE ROLE使用IF NOT EXISTS，如果不存在要创建的角色名称，则创建角色；否则不会报错。
...
示例2：创建角色role1,但是角色role1已经存在。

不使用IF NOT EXISTS，报错：指定名称的角色已经存在。

> CREATE ROLE role1; 
>
> pq: a role named role1 already exists 


使用IF NOT EXISTS，不会报错，也不会创建新的角色。

| \> CREATE ROLE IF NOT EXISTS role1; CREATE ROLE 0 |
|---------------------------------------------------|
...
SHOW ROLES语句用于查看角色列表。

##### 语法格式

SHOW ROLES的语法格式如下：

![](./assets/reference/9fd25f3cd262a1776d4705100edf8244.png)
...
GRANT \<roles\>语句可以将角色或用户添加到角色成员。添加角色成员需要admin权限。

##### 语法格式

为角色添加成员的语法格式如下：

![](./assets/reference/668982fb26f598c14049927e37b7ce4a.png)

需要注意的是：

-   角色的成员可以是用户或其他角色。

-   root用户默认为admin角色，可以为所有数据库分配权限。

-   角色的所有权限都被其所有成员继承。

-   不允许成员循环。

  如果使用WITH ADMIN OPTION，则将用户指定为角色的管理员。角色管理员可以授予或撤销指定角色的成员资格。
...
- role_name
  ...
  要将成员添加到多个角色，角色名称之间要使用逗号分隔。

- user_name

  要授予其成员资格的用户或角色的名称。

  要添加多个成员，成员名称之间使用逗号分隔。

​       可以同时将多个用户或角色加入多个角色，成为其成员。

##### 示例

示例1：将用户加入角色成为其非管理员成员。

将用户user1加入角色role1，成为role1的非管理员成员。
...
示例2：将其他角色加入角色，使其成为非管理员成员。

将角色role2加入角色role1，成为role1的非管理员成员。
...
> +-----------+--------+----------+ 
>
> role1 \| role2 \| false role1 \| user1 \| false 
>
> (2 rows) |


示例3：授予角色role1的用户成员user1管理员权限。
...
> +-----------+--------+----------+ 
>
> role1 \| role2 \| false role1 \| user1 \| true



#### 角色删除成员

​		REVOKE \<roles\>  语句可以将用户或角色成员从一个角色中删除。删除角色成员需要admin权限。需要注意的是，root用户不能从admin角色删除。

##### 语法格式

从角色删除用户或角色成员的语法格式如下：

![](./assets/reference/268e22595a3cf7179d8e0afdd11e0431.png)

如果有ADMIN OPTION FOR，则仅仅撤销角色成员的管理员角色，但是不从角色中删除其成员资格；如果没有ADMIN
OPTION FOR，则删除角色的成员。
...
- role_name
  ...
  要从中删除成员（用户或角色）的角色名称。要从多个角色中删除成员，多个角色名称之间使用逗号分隔。

- user_name

  要从角色中删除的成员（用户或角色）的名称。要删除多个成员，多个成员名称之间使用逗号分隔。

​       可以同时从多个角色中删除多个成员。

##### 示例

示例1：撤销角色role1的成员user1的管理员资格（但是不删除用户user1作为角色role1的成员资格）。
...
> REVOKE
>
> SHOW GRANTS ON ROLE role1; role_name \| member \| is_admin 
>
> +-----------+--------+----------+ 
>
> role1 \| role2 \| false role1 \| user1 \| false 
...
​		DROP ROLE 语句可以删除一个或多个角色。删除角色的用户需要拥有admin权限。需要注意的是，删除角色前，需要撤销角色的所有权限。

##### 语法格式

DROP ROLE的语法格式如下：

![](./assets/reference/a8df54fc58e22f2a9bfd5dc665820baa.png)

若不存在IF EXISTS，如果待删除的角色存在，则删除成功；否则，报错：角色不存在。

若存在IF EXISTS，如果待删除的角色存在，则删除成功；否则，不报错，也不执行删除操作。
...
删除存在的角色role1，没有IF EXISTS选项。当第二次删除role1（已经被删除）的时候，会报错：角色role1不存在。
...
> DROP ROLE role1; 
>
> pq: role role1 does not exist |


示例2：删除角色。

删除角色role2，有IF EXISTS选项。当第二次删除role1（已经被删除）的时候，不报错也不会执行删除角色的操作。
...
![](media/aa377c653fc3ccac01d853e97b720775.png)
...
![](media/09bcb5ebc45af37cd5a8e7578564d450.png)
...
![](media/3941dda4bba3f80a4c7e17e0a79f2d20.png)
...
![](media/745c3d599b6a3df5f59d9b53d0966953.png)
...
![](media/feedf64fb5bdf7915e1445c98f4bda9b.png)
